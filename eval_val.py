...
def evaluate(loader, encoder, decoder, criterion, word_map, device):
    ...
    for i, (image, caps, caplens, allcaps, image_name) in enumerate(
        ...
        while True:
            ...
            ha = decoder.linear_f(h)    
            h1, c1 = decoder.decode_step2(torch.cat([h, ha, img_f], dim = 1),(h1, c1))  
        ...
        encoder_out = encoder_out.view(1, encoder_dim)  
        encoder_out = encoder_out.expand(k, encoder_dim)  
        k_prev_words_r = torch.LongTensor([[word_map['<end>']]] * k).to(device)  # (k, 1)
        seqs_r = k_prev_words_r  # (k, 1)
        ...
        while True:
            ...
            hra = decoder.linear_r(hr)
            hr1, cr1 = decoder.decode_step_reverse2(torch.cat([hr, hra, img_r], dim = 1),(hr1, cr1))
            scores_r = decoder.fc_r(hr1)  # (s, vocab_size)
            scores_r = F.log_softmax(scores_r, dim=1)
            scores_r = top_k_scores_r.expand_as(scores_r) + scores_r  # (s, vocab_size)

            if step == 1:
                top_k_scores_r, top_k_words_r = scores_r[0].topk(k, 0, True, True)  # (s)
            else:
                top_k_scores_r, top_k_words_r = scores_r.view(-1).topk(k, 0, True, True)  # (s)

            prev_word_inds_r = top_k_words_r / vocab_size  # (s)
            next_word_inds_r = top_k_words_r % vocab_size  # (s)

            seqs_r = torch.cat([seqs_r[prev_word_inds_r], next_word_inds_r.unsqueeze(1)], dim=1)  # (s, step+1)
