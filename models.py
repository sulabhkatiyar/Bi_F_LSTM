...
class DecoderWithAttention_choice(nn.Module): 
    def __init__(self, embed_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5,choice=0, final_embeddings_dim=512, num_layers=2):
        ...
        self.linear_f = nn.Linear(decoder_dim, decoder_dim)  
        self.decode_step2 = nn.LSTMCell(decoder_dim + decoder_dim + decoder_dim, decoder_dim, bias=True)
        self.decode_step_reverse1 = nn.LSTMCell(embed_dim, decoder_dim, bias=True)
        self.linear_r = nn.Linear(decoder_dim, decoder_dim)  
        self.decode_step_reverse2 = nn.LSTMCell(decoder_dim + decoder_dim + decoder_dim, decoder_dim, bias=True)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):       
        ...
        for t in range(max(decode_lengths)):
            ...
            if self.choice==0:
                ...
                ha = self.linear_f(h[:batch_size_t])     
                h1, c1 = self.decode_step2(torch.cat([h[:batch_size_t], ha[:batch_size_t], img_f[:batch_size_t]], dim = 1),(h1[:batch_size_t], c1[:batch_size_t]))     
        ...
        for t in range(max(decode_lengths)):
            ...
            if self.choice==0:
                ...
                hra = self.linear_r(hr[:batch_size_t])
                hr1, cr1 = self.decode_step_reverse2(torch.cat([hr[:batch_size_t], hra[:batch_size_t], img_r[:batch_size_t]], dim = 1),(hr1[:batch_size_t], cr1[:batch_size_t]))     
